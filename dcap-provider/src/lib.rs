...
fn parse_certdata(
    ...
    match sig.certification_data::<quote::Qe3CertDataPpid>() {
        Ok(_)
            if sig.certification_data_type()
                != quote::CertificationDataType::PpidEncryptedRsa3072 =>
        {
            error!(
                "PPID query: Invalid certification data type: {:?}",
                sig.certification_data_type()
            );
            Err(Quote3Error::NoPlatformCertData)
        }
        Err(e) => {
            error!("PPID query: {}", e);
            Err(Quote3Error::NoPlatformCertData)
        }
        Ok(cd) => Ok(cd.clone_owned()),
    }
}
...
pub extern "C" fn sgx_ql_get_quote_config(
    ...
    match (|| {
        ...
        unsafe {
            let start = Box::into_raw(buf.into_boxed_slice()) as *mut u8;
            ptr::write(
                start as *mut Config,
                Config {

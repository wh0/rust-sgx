...
fn err_to_result(err: u32) -> Result<(), Quote3Error> {
...
pub fn target_info() -> Result<Targetinfo, Quote3Error> {
    unsafe {
        ...
        err_to_result(get_target_info(&mut targetinfo))?;
        Ok(targetinfo)
    }
}
...
pub fn quote(report: &Report) -> Result<Vec<u8>, Quote3Error> {
    unsafe {
        ...
        err_to_result(get_quote_size(&mut quote_size))?;

        let mut quote = vec![0; quote_size as _];
        err_to_result(get_quote(&report, quote_size, quote.as_mut_ptr()))?;
        Ok(quote)
    }
}
